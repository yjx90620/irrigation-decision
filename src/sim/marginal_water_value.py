...
Method: start from a moderate baseline SMT=[50,50,50,50] for both crops,
then for each (crop, stage) in turn raise that one stage's SMT by
+15 percentage points (holding the other 3 stages and the other crop's
schedule fixed), rerun the rotation, and record

    MWV(crop, stage) = delta(system yield) / delta(irrigation applied)
