...
CONDITION_LABELS = {"zero_shot": "零样本迁移", "finetuned": "目标域微调", "threshold_rule": "规则基线"}
CONDITION_COLOR = {"zero_shot": PALETTE[0], "finetuned": PALETTE[1], "threshold_rule": "gray"}
...
def main():
    ...
    fig.suptitle("论文三 对比四：轮作版留一迁移 + 两因子风险验证（audit-v2 物理，2018—2022）", fontsize=13, y=1.03)
    ...
    for i, cond in enumerate(["zero_shot", "finetuned", "threshold_rule"]):
        ...
        ax.bar(x + (i - 1) * width, vals, width, label=CONDITION_LABELS[cond], color=CONDITION_COLOR[cond])
    ax.set_xticks(x)
    ax.set_xticklabels([SITE_LABELS_CN[s] for s in ORDER], rotation=20)
    ax.set_ylabel("总产量 (t/ha)")
    ax.set_title("(a) 各目标站点三种条件产量")
    ax.legend(fontsize=9)

    # panel (b): two-factor risk vs actual gap (the r=0.95 validation)
    ax = axes[1]
    ...
    ax.set_title(f"(b) 两因子风险 vs 实测缺口  r={r:.3f}（n=5，方向性证据）")
