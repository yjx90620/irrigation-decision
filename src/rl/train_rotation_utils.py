...
def _make_env(rank, sites, mode, soils=("loam",), years=TRAIN_YEARS,
              config: RLExperimentConfig = PRIMARY_CONFIG):
    # P0-4: one fixed site per worker rank (see RotationGymEnv.fixed_site).
    return RotationGymEnv(
        mode=mode, sites=list(sites), soils=list(soils), years=list(years),
        fixed_site=sites[rank % len(sites)], config=config,
    )
...
def train_rotation_policy(
    ...
    """audit-v3 (2.1/3.11): one config drives reward gamma/water-normalizer
    AND the training device/seed - no more separate water_norm/device
    kwargs, and the finetune path (PPO.load) uses the same device as the
    fresh-training path (no source/finetune device mismatch)."""
    config.validate()
    n_envs = workers_per_site * len(sites)
    env_fns = [functools.partial(_make_env, rank, sites, mode, config=config) for rank in range(n_envs)]
    vec_env = SubprocVecEnv(env_fns)

    if base_model_path and base_vecnormalize_path:
        vec_env = VecNormalize.load(str(base_vecnormalize_path), vec_env)
        vec_env.training = True
        model = PPO.load(str(base_model_path), env=vec_env, device=config.device)
    else:
        vec_env = VecNormalize(vec_env, norm_obs=True, norm_reward=False, clip_obs=10.0)
        model = PPO(
            "MlpPolicy", vec_env, verbose=0, n_steps=512, batch_size=256, n_epochs=10,
            learning_rate=3e-4, gamma=config.gamma, ent_coef=ent_coef, device=config.device,
        )
