...
> ## ⚠️ 重要：原型阶段结果已被取代，不要直接引用
>
> 本文档下半部分"基线实验网格结果""NSGA-II 帕累托前沿"以及 `processed/` 里对应的
> `baseline_experiment_results.csv`、`pareto_front_*.csv`、PPO checkpoint、迁移实验结果，
> **全部是在一个已被证实存在根本性问题的实验设置下产生的**：单季夏玉米 + 每季从田间持水量起算，
> 导致 5 个站点里有 4 个不灌溉就能拿到 93—99% 的产量，全部 5400 次模拟里 75% 的产量挤在
> 0.76 t/ha 的窄带内——**灌溉在那个设置下几乎不影响结果**。
>
> 详细诊断和修复方案见 [docs/系统升级方案.md](../docs/系统升级方案.md)。修复后（冬小麦—夏玉米轮作
> + ERA5-Land 真实初始墒情）动态范围提升到 4—5 倍。
>
> 这些数据**保留不删**，因为：①"固定FC假设把雨养产量高估54%"本身是论文一的一组对比结果，
> 需要原始数据支撑；② PPO 两次训练失败的调试记录是论文二的方法学材料。
> 但它们**不能作为最终结论引用**，正式结果要等升级后重跑。

## 目录结构

- `raw/weather/` — Open-Meteo 全周期气象数据（主数据源）
- `raw/weather_agera5/` — AgERA5 验证样本（2024年，5站点×4变量）
- `raw/soil_moisture/` — ERA5-Land 多层土壤湿度（5站点×1981—2025），用于真实初始墒情
- `raw/cmip6/` — CMIP6 HighResMIP 气候情景（7模式集合，历史期1991—2010 + 未来期2031—2050）
